...
Original file is located at
    https://colab.research.google.com/drive/1_dSDcTtFx5vMLBeIBn0mwfteD6GYR3fs
"""
...
def teste_t(df, clas, cor, variavel):
  # teste t
  g1 = df[df[clas]<10]
  g2 = df[df[clas]>=10]
  desv_pad_g1 = g1[variavel].std()
  desv_pad_g2 = g2[variavel].std()
  n_g1 = len(g1)
  n_g2 = len(g2)
  med_g1 = g1[variavel].mean()
  med_g2 = g2[variavel].mean()

  s2 = ((n_g1-1)*desv_pad_g1**2) + ((n_g2-1)*desv_pad_g2**2)
  s2 = s2/(n_g1+n_g2-2)

  t = (med_g1 - med_g2/(np.sqrt(s2*(1/n_g1 + 1/n_g2))))

  t_tab = stats.t.ppf(q = 0.925, df = n_g1+n_g2-2)

  # se t >= t_tab rejeita-se a hipótese nula (hipótese em que não ha diferença entre as médias)
  # se t tab < t_tab aceita-se a hipósete nula (hipótese em que não ha diferença entre as médias)
  classfc = ['g1' if i<10 else 'g2' for i in df[clas]]
  df['classfc'] = classfc

  tabl = df.groupby('classfc')[variavel].describe()

  figura, axes = plt.subplots(1,2, figsize = (13,5), width_ratios = [1,0.8])

  sns.boxplot(ax = axes[0], data = df, x = 'classfc', y = variavel, color = cor)
  axes[1].text(0.1,0.27,f'obtained t: {t.round(3)}\ntabled t: {t_tab.round(3)}', backgroundcolor = 'lightgreen')
  axes[1].set(xticks = [], yticks = [])
  axes[0].set_xlabel('')
  tabela = axes[1].table(cellText = tabl.values.round(3), bbox = [0,0.4,1,.5], rowLabels = tabl.index, colLabels = tabl.columns)
...
def controle(df):
    ...
    figura, axes = plt.subplots(2,2, figsize = (15,5), width_ratios = [1,0.7], height_ratios=[1,0.4])

    gs = axes[0,0].get_gridspec()
    [i.remove() for i in axes[0:,0]]
    axes[0,0] = plt.subplot(gs[0:,0])

    descr = df.describe()
    descr =  pd.concat([descr, pd.DataFrame([descr.loc['max'] - descr.loc['min']], index = ['ampl'])])
    sns.pointplot(ax = axes[0,0], data = df,)
    CEP = pd.read_excel('CEP.xlsx')
    LSC = descr.loc['mean'].mean() + ((descr.loc['ampl'].max()-descr.loc['ampl'].min())*CEP.iloc[-1,1])
    LIC = descr.loc['mean'].mean() - ((descr.loc['ampl'].max()-descr.loc['ampl'].min())*CEP.iloc[-1,1])
    axes[0,0].axhline(y = LSC, color = 'black')
    axes[0,0].axhline(y = LIC, color = 'black')
    axes[0,0].axhline(y = 2.97, color = 'green')
    axes[0,0].axhline(y = 2.83, color = 'green')
    axes[0,0].axhline(y = 3.03, color = 'y')
    axes[0,0].axhline(y = 2.77, color = 'y')

    lista = []
    for i in df.columns:
      for j in df[i]:
        lista.append(j)
    sns.histplot(ax = axes[0,1], data = lista)
    ...
    results = stats.anderson(lista)
    stat_critica = results.critical_values[2]
    stat_obtencao = results.statistic
    #se a estatística calculada for menor que a estatística crítica os dados seguem a normal
    axes[1,1].set(xticks = [], yticks = [])
    axes[1,1].spines[['top','bottom','left','right']].set_visible(False)
    axes[1,1].text(0.1,0.07,f'R2: {R2.round(3)}\nObtained F: {F.round(3)}\nTabled F: {Ftab.round(3)}\nStatiltical: {stat_obtencao.round(3)}\nCritical: {stat_critica.round(3)}', backgroundcolor = 'lightblue')
